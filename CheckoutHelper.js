...
CheckoutHelperJS={
	...
	franjaProductoEspecial:"",
	
	pendingOrderId: "",
	
	setDateTimeDefaultValue:function(dateTimeDefaultValue){
		this.dateTimeDefaultValue=dateTimeDefaultValue;
	},	

	getDateTimeDefaultValue:function(){
		return this.dateTimeDefaultValue;
	},	
	
	setCurrentContractId:function(currentContractId){
	...
	//	this.tempOrderTotal = orderTotal + 0;
		this.tempOrderTotal = orderTotal;
	...
	deleteFromCart:function(orderItemId,forWishlist){ 
		...
		params.shopCartStep = this.getShopCartStep();
		params.skipCheckInventory = "1";		
	...
	updateAddressIdOFItemsOnCreateEditAddress:function(addressId){
		...
		params.remerge="***";
		params.check="*n";
		params.updateRequestedShipDate="1";
	...
	applyPromotionCode:function(formName,returnView) {
		...
		if (trim(form.promoCode.value) == "") {
			/* GATracking */
			GAFunctions.eventPush('cupon','error-form','codigo-vacio');
			/* GATracking */
	...
	checkoutOrder:function(orderId,userType,addressListForMailNotification,isQuote){
		...
		var lastSalesAssistantTmp = getCookie("lastSalesAssistant");

		if(lastSalesAssistantTmp!=null){
			params["lastSalesAssistantTmp"]=lastSalesAssistantTmp;
		}
	...
	validateDate: function(dateObj, elementId){
		...
		if(dateObj.getValue() != null && (dateObj.compare(dateObj.getValue(), now) < 0)){	
			...
			if((elementId != null) && (document.getElementById(elementId) != null)){
				...
				GAFunctions.eventPush('error', 'error-form', document.getElementById(elementId).id+'-'+MessageHelper.messages["PAST_DATE_ERROR"]);
			}else{
	...
	createAddress:function(orderItemId,addressType){
		...
		this.showHideDivs('editAddressContents','WC_ShipmentDisplay_div_16');
		this.hideDivHeader('WC_ShipmentDisplay_div_5');
	},
	...
	hideDivHeader:function(hideArea){
		document.getElementById(hideArea).style.display = "none";		
	},
	showDivHeader:function(showArea){		
		document.getElementById(showArea).style.display = "block";
	},
	/**
			...
			if(this.shipmentTypeId == "2"){
				...
				var element = document.getElementById("singleShipmentAddress");
		 		if (element != null && document.getElementById("addressId_all")) {
		 			...
		 			//alert("addressId_all=T"+document.getElementById("addressId_all").value+"T");
		 			
		 			//if(document.getElementById("addressId_all").value){
		 			//alert("addressId_all11=T"+document.getElementById("addressId_all_ext").value+"T")
		 			//}
		 			
		 			//element.value = document.getElementById("addressId_all_ext").value;
										
		 			//var x=element;
		 			//var txt="All options: "+document.getElementById("addressId_all_ext").value;
		 			//var i;
	...
	*
       */
 	reEditAddress:function(addressSelectBoxName,orderItemId,profileshipping,profilebilling){
	  var addressBox = document.getElementById(addressSelectBoxName);
		// Start changes for address drop down issue fix
		if(isNaN(addressBox.value)){
			return false;
		}
		...
		cursor_wait();
		// Start to refresh the geo map details
		cursor_wait();
		var params = [];
		params["storeId"] = this.storeId;
		params["catalogId"] = this.catalogId;
		...
		params["nivelBusquedas"]="2";
		params["cantidadResultadosMaximo"]="-1";
		params["filtrosLibres"]="false";
		params["habilitarSimilaridad"]="true";
		params["serviceType"]="buscarDomicilioAvanzado";
		params["soapAction"]="http://unisolutions.com.ar/buscarDomicilioAvanzado";
		wc.service.invoke("AjaxCencosudGeoAddressServiceCmd", params);
	...
	editAddress:function(addressSelectBoxName,orderItemId,profileshipping,profilebilling){
		...
		// end changes for address drop down issue fix
	...
	cancelEditAddress:function(){
		this.showHideDivs('WC_ShipmentDisplay_div_16','editAddressContents');
		this.showDivHeader('WC_ShipmentDisplay_div_5');
	 ...
	 */	
	showHideCheckouftSMS:function(checkboxElementId, smsElementId){
	...
	updateCartWaitCencosud:function(objectQuantityId, orderItemId, event, accion) {

		quantityBox = dojo.byId( objectQuantityId );	
	...
	updateCart:function(quantityBox, orderItemId,keyPressCountValue){
		...
		if(!isNonNegativeInteger(quantity)){
			...
			params["langId"] = this.langId;
			params.calculationUsage = "-1,-2,-3,-4,-5,-6,-7";
			params["skipFulfillmentValidations"]="1";
			this.updateParamObject(params,"orderItemId",orderItemId,false,-1);
	...
	updateAddressForAllItems:function(addressSelectBox){
		...
		var addressId = document.getElementById("singleShipmentAddress").value;
		setCurrentId("singleShipmentAddress");
		this.updateParamObject(params,"addressId",addressId,false);
		...
		var divShipInstructions='';
		if(dojo.byId('divDefaultTextInstruction')!=null)
			divShipInstructions=dojo.byId('divDefaultTextInstruction').innerHTML
		
		if(!enabledShipInstructions || shipInstructions == undefined || shipInstructions == "" || shipInstructions == divShipInstructions) {
			wc.service.invoke("OrderItemAddressShipMethodUpdate", params);               		
		} else {
	...
	preUpdateShipModeForAllItems:function(shipmentSelectBox){		
		document.getElementById('shippingBillingPageNext').disabled=true;
		this.hideErrorMessage();		
		var zonaGeograficaIdForAddress = dojo.byId('zonaGeograficaIdForAddress');
		var selectedShipmode = document.getElementById('singleShipmentShippingMode').options[document.getElementById('singleShipmentShippingMode').selectedIndex].text;
		if(zonaGeograficaIdForAddress.value=='3'){
			if(selectedShipmode == 'Programada')
			{
				
				this.showShippingModeProgramadoZone3(null);
				this.updateShipModeForAllItems(shipmentSelectBox);	
			}
			else
			{
				try{
					 dojo.byId("divShipingDate").style.display = 'none';	
					  }catch(e){}
				this.showShippingModeFree(shipmentSelectBox);
				
			}
			
		}else{
			if( this.isListDays() ){
				if( this.isShippingModeNormal() ){
					if(this.showShippingModeNormal(null, true)){
						var widget = dijit.byId('requestedEasyShippingDate');
						setCurrentId('requestedEasyShippingDate');
						this.updateRequestedShipDate(widget, shipmentSelectBox);
					}				
				}else if(this.isShippingModeProgramado()){
					this.showShippingModeProgramado(null);
					this.updateShipModeForAllItems(shipmentSelectBox);				
				}
			}		
		}
		document.getElementById('shippingBillingPageNext').disabled=false;
	},	
	/**
	...
	updateShippingInstructionsForAllItems:function(){
		...
		var defaultInstruction = dojo.byId('divDefaultTextInstruction').innerHTML
		if(defaultInstruction == shipInstructions){
			shipInstructions = ""
		}		
		shipInstructions.replace(/DIRECCION = COUNTRY/g,"")
		var n = shipInstructions.search("/DIRECCION = COUNTRY/");
		

		var visSetting = (dojo.byId('chkboxCountry').checked) ? "SI" : "NO";
		
		var countrybarrio = ""
		if(visSetting=="SI"){
		 countrybarrio = dojo.byId('divCountryBarrio').innerHTML;		
		}else{
			countrybarrio = "";
		}
		...
		if(countrybarrio!=null && countrybarrio!="" ){
			if (n>0){
				var inicio = shipInstructions.substring(0,n)
			shipInstructions = shipInstructions.substring(n,21)
			}
			shipInstructions += countrybarrio;
		}else{
			if (n>0)
			shipInstructions = shipInstructions.substring(n,21)
		}
	...
	updateRequestedShipDate:function(jsDate, shippingMode){
		
		if(jsDate == null || jsDate.getDisplayedValue() == null)
			return;
		
		var franjaHoraria	= dojo.byId("franjaHoraria");
		
			if( franjaHoraria.value!=this.itemsFranja[0].id && franjaHoraria.value!=this.itemsFranja[1].id ){
				return null;
			}
		...
		if(jsDate.getDisplayedValue() != ""){
			...
			if( franjaHoraria.value==this.itemsFranja[0].id ){
				t.setHours(6);				
			}else{
				t.setHours(18);				
			}
		...
		this.updateShippingDateForAllItems(t,shippingMode);
	},
	 ...
	 */
	updateShippingDateForAllItems:function(date, shippingMode){
		params = [];
		params["storeId"] = this.storeId;
		params["catalogId"] = this.catalogId;
		params["langId"] = this.langId;
		...
		params.remerge="***";
		params.check="*n";
		params.method="updateShippingDateForAllItems";
		
		var requestedShipAddr=dojo.byId("singleShipmentAddress");
		if(requestedShipAddr != null){
		params.addressId=requestedShipAddr[requestedShipAddr.selectedIndex].value;
		}
				
		this.updateParamObject(params,"method2","updateShippingDateForAllItems",false);
		this.updateParamObject(params,"method3","updateShippingDateForAllItems",true);
		
		var requestedShippingDate=dojo.byId("requestedEasyShippingDate");
		
		if(requestedShippingDate != null ){
			this.updateParamObject(params,"requestedCencosudShipDate",requestedShippingDate.value,false);	
		}
		
		var franjaHoraria	= dojo.byId("franjaHoraria");
		if(franjaHoraria != null ){
			this.updateParamObject(params,"franjaHoraria",franjaHoraria.value,false);	
		}
		...
		if(shippingMode!=null&&shippingMode!=undefined){
			this.updateParamObject(params,"shipModeId",shippingMode.value,false);			
		}
	...
	itemsFranja: [{"id":"AM","value":"de 8:00 a 14:00hs"},{"id":"PM","value":"de 12:00 a 20:00hs"}],	
	
	searchListDays:function(orderIdTemp){
		tempListDays = null;
		var xhrArgs = {
			url: "/webapp/wcs/stores/servlet/CencosudListShippingDaysView?orderId="+orderIdTemp,				  
			handleAs: "json",
			load: function(data){
				tempListDays = data.items;
			},
	...
	isShippingModeNormal: function(){
		var optionShippingMode = this.getShippingMode("singleShipmentShippingMode");
		var codeTemp = null;
		try{
			codeTemp = optionShippingMode.code
		}catch (e) {}
		return codeTemp == 'Normal';
	},
	
	isShippingModeProgramado:function(){
		var optionShippingMode = this.getShippingMode("singleShipmentShippingMode");
		var codeTemp = null;
		try{
			codeTemp = optionShippingMode.code
		}catch (e) {}
		return codeTemp == 'Programada';		
	},
	
	showShippingModeFree:function(shipmentSelectBox){
		dojo.byId('msgDevliveryOrder').style.display='block';
		try{
			dojo.byId("divShipNormalMsg").style.display = 'none';	
		}catch(e){}
		
		var shippingDate	= dojo.byId("requestedEasyShippingDate");
		shippingDate.value  = dojo.byId("formattedReqShipDateZona3").value;
		
		var widget = dijit.byId('requestedEasyShippingDate');
		setCurrentId('requestedEasyShippingDate');		
		dojo.byId('franjaHoraria').value=dojo.byId('franjaHorariaZona3').value;
		this.updateRequestedShipDate(widget, shipmentSelectBox)		
	},
	
	showShippingModeNormal:function( currentDateFull, recalculate){
		var divShipingDate		= dojo.byId("divShipingDate");
		var divFranjaProgramada = dojo.byId("divFranjaProgramada");
		var divFranjaNormal 	= dojo.byId("divFranjaNormal");		
		
		var shippingDate	= dojo.byId("requestedEasyShippingDate");					
		var franjaNormal 	= dojo.byId("franjaNormal");
		var franjaHoraria 	= dojo.byId("franjaHoraria");
		...
		var timeSlots=null;
		//var tempDataDays=this.tempListDays[0].split("|");		
		var tempDataDays=this.normalDay.split("|");		
		if(recalculate){			
			currentTime=tempDataDays[0];
			timeSlots=tempDataDays[1];
		}else{
			currentTime=currentDateFull;
			timeSlots=franjaHoraria.value;			
		}
		...
		shippingDate.value=currentTime; 		
		
		if( this.itemsFranja[0].id == timeSlots ){
			franjaNormal.value = this.itemsFranja[0].value;
			franjaHoraria.value = this.itemsFranja[0].id;				
		} if( this.itemsFranja[1].id == timeSlots ){
			franjaNormal.value = this.itemsFranja[1].value;					
			franjaHoraria.value = this.itemsFranja[1].id;
		}
		
		var especialProduct = document.getElementById("statusForEspecialProduct");
		if(especialProduct.value=='Y'){
			franjaNormal.value = this.franjaProductoEspecial;
		}		
		
		divFranjaNormal.style.display		= "block";
		divShipingDate.style.display		= "none";
		divFranjaProgramada.style.display	= "none";
		dojo.byId("divShipNormalMsg").style.display = 'block';
	...
	showShippingModeProgramado: function( currentDateFull ){
		var divShipingDate		= dojo.byId("divShipingDate");		
		var divFranjaProgramada = dojo.byId("divFranjaProgramada");
		var divFranjaNormal 	= dojo.byId("divFranjaNormal");		
		
		var shippingDate		= dojo.byId("requestedEasyShippingDate");							
		var franjaProgramada	= dojo.byId("franjaProgramada");
		var franjaHoraria 		= dojo.byId("franjaHoraria");			
      
		dojo.byId("divShipNormalMsg").style.display = 'none';
		
		divFranjaProgramada.style.display	= "block";			
		divShipingDate.style.display		= "block";
		divFranjaNormal.style.display 		= "none";				
	
		this.updateShippingFranjaList( currentDateFull );
		...
		if(franjaHoraria.value==this.itemsFranja[1].id){
			selectedIndex=2;					
		}
							
		franjaHoraria.value= "";		
		if( franjaProgramada.length==1 ){
			shippingDate.value = 'Elegir fecha';
		}else if(franjaProgramada.length>1 && selectedIndex<franjaProgramada.length){
			if( currentDateFull!=null ){
				shippingDate.value = currentDateFull;
				franjaProgramada.selectedIndex = selectedIndex;				
			}
		}else{
			if( currentDateFull!=null ){
				shippingDate.value=currentDateFull;				
	...
	showShippingModeProgramadoZone3: function( currentDateFull ){
		var divShipingDate		= dojo.byId("divShipingDate");		
		var divFranjaProgramada = dojo.byId("divFranjaProgramada");
		var divFranjaNormal 	= dojo.byId("divFranjaNormal");		
		...
		var franjaProgramada	= dojo.byId("franjaProgramada");
		var franjaHoraria 		= dojo.byId("franjaHoraria");
        var divmsgDevliveryOrder = dojo.byId("msgDevliveryOrder");		
        
		divFranjaProgramada.style.display	= "block";			
		divShipingDate.style.display		= "block";
		divFranjaNormal.style.display 		= "none";				
        divmsgDevliveryOrder.style.display="none";
		this.updateShippingFranjaList( currentDateFull );
	...
	updateShippingFranjaList: function( currentDate ){
		var franjaProgramada = dojo.byId("franjaProgramada");		
		this.removeOptions(franjaProgramada);
 		var listDates=document.getElementById("listDates").value;
		if(listDates.indexOf(currentDate+'|'+this.itemsFranja[0].id)>-1){
			franjaProgramada.options[franjaProgramada.length] = new Option( this.itemsFranja[0].value, this.itemsFranja[0].id);
		}
		if(listDates.indexOf(currentDate+'|'+this.itemsFranja[1].id)>-1){
			franjaProgramada.options[franjaProgramada.length] = new Option( this.itemsFranja[1].value, this.itemsFranja[1].id);
		}
		/*		
		if( dojo.indexOf( this.tempListDays, currentDate+'|'+this.itemsFranja[0].id )>-1 ){
			franjaProgramada.options[franjaProgramada.length] = new Option( this.itemsFranja[0].value, this.itemsFranja[0].id);
			
		}
		
		if( dojo.indexOf( this.tempListDays, currentDate+'|'+this.itemsFranja[1].id )>-1 ){
			franjaProgramada.options[franjaProgramada.length] = new Option( this.itemsFranja[1].value, this.itemsFranja[1].id);
		}*/
		franjaProgramada.value = "";
	},
	initShippingInfo:function( ){
		var zonaGeograficaIdForAddress = dojo.byId('zonaGeograficaIdForAddress');		
		if(zonaGeograficaIdForAddress.value=='3'){
			
			if(dojo.byId('franjaHoraria')!= null && dojo.byId('franjaHoraria').value=='AM'){
				dojo.byId('msgDevliveryOrder').style.display='block';
			}			
		}else{
			if(this.isListDays()){
				var tempRequestedShippingDate = dojo.byId("tempRequestedEasyShippingDate");
				var currentDateFull = null;				
				if( tempRequestedShippingDate!=null ){
					currentDateFull = tempRequestedShippingDate.value; 
				}
				
				if( this.isShippingModeNormal() ){			
					this.showShippingModeNormal( currentDateFull );
				}else if( this.isShippingModeProgramado() ){
					this.showShippingModeProgramado( currentDateFull );	
				}			
			}else{
				var singleShipmentShippingMode = dojo.byId("singleShipmentShippingMode");			
				singleShipmentShippingMode.value="";
			}
		}		
	},	
	initProgramadaShipInfo:function()
	{
		var zonaGeograficaIdForAddress = dojo.byId('zonaGeograficaIdForAddress');		
		if(zonaGeograficaIdForAddress.value=='3'){	
			
			dojo.byId('divShipingDate').style.display='block';
		}
		var currentDateFull = null;
		var tempRequestedShippingDate = dojo.byId("tempRequestedEasyShippingDate");
		if( tempRequestedShippingDate!=null ){
	...
	isListDays:function(){	
		if( this.tempListDays==null || !this.tempListDays.length || this.tempListDays.length==0 ){
			return false;
		}
		return true;
	},
	
	preUpdateRequestedShipDate:function( franja ){
		this.hideErrorMessage();		
		var shippingDate	= dojo.byId("requestedEasyShippingDate");		
		var especialProduct = dojo.byId("statusForEspecialProduct");
		var tempProgramadaShipName = dojo.byId("tempDefaultProgramadaName");
		var zonaGeograficaIdForAddress = dojo.byId('zonaGeograficaIdForAddress');		
		...
		if( especialProduct.value == 'Y' ){
			...
			this.removeOptions(franjaProgramada);		
			franjaProgramada.options[franjaProgramada.length] = new Option( this.franjaProductoEspecial, 'AM');
			franjaProgramada.value = 'AM';
			this.updateRequestedShipDateFranja(franja);		
					
		}else{
			this.updateShippingFranjaList( shippingDate.value );
		}			
	},
	 getBOCRequestCall:function(storeID,orderID){
		var params = [];
		...
		cursor_wait();
		wc.service.invoke("AjaxCencosudBOCRequestCmd", params);	
	},
	
	updateRequestedShipDateFranja:function( franja ){
		this.hideErrorMessage();		
		var widget = dijit.byId('requestedEasyShippingDate');		
		var franjaProgramada	= dojo.byId("franjaProgramada");
		var franjaHoraria 		= dojo.byId("franjaHoraria");
		var addressId = document.getElementById("singleShipmentAddress").value;
		if(isNaN(addressId)){
			MessageHelper.displayErrorMessage("Por favor seleccioná una dirección de entrega");
			return false;
		}
		franjaHoraria.value = franjaProgramada.value;
		this.updateRequestedShipDate( widget );
	},	
	shopCartStep:"",
	getShopCartStep:function(){
		return this.shopCartStep;
	},
	setShopCartStep:function(param){
		this.shopCartStep=param;
	},
	
	changeContractForLocale:function(){
		//redirect(dojo.byId('newContractId').value,,"1","AjaxOrderItemDisplayView");
		var cookieValue = dojo.cookie("location");
		if (cookieValue != null){
			dojo.cookie("location", null, {expires: -1, path: "/"});
		}
		dojo.cookie("location", dojo.byId("locationForAddress").value, {path: "/"});

		this.redirectShoppingFlow();
	},
	
	redirectShoppingFlow : function (  ){
		document.location.href=dojo.byId("newUrlForContractChange").value;
	},
	
	updateParameterInstallment:function( url ){
		var urlFinal = url;
		if( url.indexOf('&numberOfInstallments')>0 ){			
			urlFinal = url.substring(0,url.indexOf('&numberOfInstallments'));
			//var rest = url.substring(url.indexOf('&numberOfInstallments')+1);
			//if(rest.indexOf("&")>0){
			//	urlFinal = rest.substring(rest.indexOf("&"));
			//}
		}
		
		var installment = dojo.byId("numberOfInstallments_1");
		
		if( installment!=null && installment.value!=null && installment.value!="" && installment.value!="1"  ){
			urlFinal=urlFinal+"&numberOfInstallments="+installment.value;
			urlFinal=urlFinal+"&payMethodId="+dojo.byId("payMethodId_1").value;
			urlFinal=urlFinal+"&updateAmount=1";			
		}
					
		return urlFinal;		
	}, 
	refreshShippingMethod:function( addressId, statusForProductType){
		if(statusForProductType!='1'){
			var widget=dijit.byId('shippingMethodDetailsArea');
			widget.refresh({"addressId":addressId});
		}
	},
	validateUserToChooseFlow:function(){
		var shopOnlineVal=document.getElementById("shipTypeOnline").checked;
		
		var pickInStoreVal=document.getElementById("shipTypePickUp").checked;
		
		if (shopOnlineVal==false && pickInStoreVal==false){
		MessageHelper.displayErrorMessage("Por Favor selecciona una opcion de despacho.");
		return false;
		}
		return true;
	...
	validateContract:function(storeId,catalogId,orderId,langId){
		var params = [];
		params["storeId"]= storeId;
		params["catalogId"]= catalogId;
		params["orderId"]= orderId;
		params["langId"]= langId;
		wc.service.invoke("AjaxCencosudOrderValidationCmd", params);	
		
	},
	
	validateShippingPage:function(){
		var addressId = document.getElementById("singleShipmentAddress").value;
		if(isNaN(addressId)){
			MessageHelper.displayErrorMessage("Por favor seleccioná una dirección de entrega");
			return false;
		...
		var zonaGeograficaIdForAddress=dojo.byId("zonaGeograficaIdForAddress");
		var statusForProductType=dojo.byId("statusForProductType");		
		
		if(zonaGeograficaIdForAddress.value!='3'&&statusForProductType.value!='1'){
			var singleShipmentShippingMode=dojo.byId("singleShipmentShippingMode");			
			var requestedShippingDate=dojo.byId("requestedEasyShippingDate");
				
			
			if(singleShipmentShippingMode.value==""){
				this.displayErrorMessage(MessageHelper.messages["ERROR_ShippingModeEmpty"]);
				GAFunctions.eventPush("envio", "error-form",singleShipmentShippingMode.id+"-"+MessageHelper.messages["ERROR_ShippingModeEmpty"]);
				return false;				
			}

			if(requestedShippingDate.value==""||CheckoutHelperJS.getDateTimeDefaultValue()==requestedShippingDate.value){
				this.displayErrorMessage(MessageHelper.messages["ERROR_ShippingTimeEmpty"]);
				GAFunctions.eventPush("envio", "error-form",singleShipmentShippingMode.id+"-"+MessageHelper.messages["ERROR_ShippingTimeEmpty"]);
				return false;
			}
			
			if( this.isShippingModeNormal() ){
				var franjaNormal=dojo.byId("franjaNormal");				
				if(franjaNormal.value==""){
					this.displayErrorMessage(MessageHelper.messages["ERROR_TimeSlotsEmpty"]);					
					return false;					
				}				
			}else{
				var franjaNormal=dojo.byId("franjaProgramada");				
				if(franjaNormal.value==""){
					this.displayErrorMessage(MessageHelper.messages["ERROR_TimeSlotsEmpty"]);
					return false;					
				}								
		...
		var contractIdForAddress=dojo.byId("contractIdForAddress");
		var localidadValida		=dojo.byId("localidadValida");
		/*if(this.currentContractId != contractIdForAddress.value || localidadValida.value != "1" ){			
			this.changeContractForLocale();
			return false;
		}*/
		
		if( dojo.byId("totalShippingCharge") && ! ( dojo.byId("totalShippingCharge").value > 0) ){
			console.debug("CheckoutHelperJS.validateShippingPage() totalShippingCharge "+dojo.byId("totalShippingCharge").value);
			MessageHelper.displayErrorMessage("No es posible entregar tu pedido en esta dirección, por favor seleccioná otra dirección o hacé click en VOLVER y elegí la opción de RETIRO EN TIENDA.");
			return false;
		}
		...
		if(document.getElementById('requestedEasyShippingDate').value != null && document.getElementById('requestedEasyShippingDate').value != '') {
			GAFunctions.eventPush("seleccionar", "fecha-entrega", GAFunctions.getDifferenceBetweenDates(new Date(), document.getElementById('requestedEasyShippingDate').value));
		}
			CheckoutHelperJS.validateContract(this.storeId,this.catalogId,this.orderId,this.langId);
		//document.location.href="OrderShippingBillingView?langId="+this.langId+"&storeId="+this.storeId+"&catalogId="+this.catalogId+"&forceShipmentType="+CheckoutHelperJS.getShipmentTypeId()+"&tipoPagina=P";		
	},
	
	showPopupChangedLocation : function(){
		$('#contenedorId').jqm({modal:true});
		$('#contenedorId').jqmShow();
	},
	displayErrorMessage:function(msg){
		dojo.byId('errorMessageArea').innerHTML = msg;
		dojo.byId('errorMessageArea').style.display	="block";
		dojo.byId('espacioMessageArea').style.display="none";		
	},
	hideErrorMessage:function(){
		dojo.byId('errorMessageArea').style.display="none";
		dojo.byId('espacioMessageArea').style.display="block";		
	},
	lookForRecentOrders: function(){
		var params = [];
		params["storeId"] = this.storeId;
		wc.service.invoke('AjaxCencosudLookForRecentOrdersCmd',params)
	},
	showPendingOrderConfirmationPage:function(){	
		document.location.href="OrderShippingBillingConfirmationView?langId="+this.langId+"&storeId="+this.storeId+"&catalogId="+this.catalogId+"&shipmentTypeId="+"1"+"&orderId="+this.pendingOrderId;
	},
	
	displayMessageRecentOrderFound: function(orderId){
		if(dojo.cookie("msgRecentOrderFound") != "yes"){
			$("#idPedido").html(orderId);
			$("#ErrorMessageText").html($("#messageForRecentOrderFound").html())
			$("#MessageArea").show();
			dojo.cookie("msgRecentOrderFound", "yes", {path: "/"});
		}
	}
